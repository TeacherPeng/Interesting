...
Namespace Shapes
	Public Class Sphere
		...
		Public Property ThetaDiv() As Integer
			Get
				Return CInt(Fix(GetValue(ThetaDivProperty)))
			End Get
		...
		Public Property PhiDiv() As Integer
			Get
				Return CInt(Fix(GetValue(PhiDivProperty)))
			End Get
		...
		Private Shared Function GetTextureCoordinate(ByVal theta As Double, ByVal phi As Double) As Point
			Dim p As New Point(theta / (2 * Math.PI), phi / (Math.PI))
