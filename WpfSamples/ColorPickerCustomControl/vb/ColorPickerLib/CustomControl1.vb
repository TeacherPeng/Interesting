...
Namespace ColorPickerLib
	...
	Public Class ColorPicker
		...
		Public Custom Event SelectedColorChanged As RoutedPropertyChangedEventHandler(Of Color)
			...
			End RemoveHandler
			RaiseEvent(ByVal sender As Object, ByVal e As RoutedPropertyChangedEventArgs(Of Color))
			End RaiseEvent
		End Event
		...
		Public Shared ReadOnly SelectedColorProperty As DependencyProperty = DependencyProperty.Register ("SelectedColor", GetType(Color), GetType(ColorPicker), New PropertyMetadata(Colors.Transparent, New PropertyChangedCallback(AddressOf selectedColor_changed)))
		...
		Protected Overridable Sub OnHexadecimalStringChanged(ByVal oldValue As String, ByVal newValue As String)
			...
			Try
				...
				If shouldFindPoint AndAlso (Not isAlphaChange) AndAlso templateApplied Then
					updateMarkerPosition(m_color)
				End If
			Catch fException As FormatException
		...
		Private Sub ColorDetailSizeChanged(ByVal sender As Object, ByVal args As SizeChangedEventArgs)
			...
			ElseIf m_ColorPosition IsNot Nothing Then
				markerTransform.X = (CType(m_ColorPosition, Point)).X * args.NewSize.Width
				markerTransform.Y = (CType(m_ColorPosition, Point)).Y * args.NewSize.Height
			End If
		End Sub
		...
		Private templateApplied As Boolean
