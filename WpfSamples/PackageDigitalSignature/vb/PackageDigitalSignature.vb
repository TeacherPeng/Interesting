...
Imports System.Security.Cryptography ' for CryptographicException
Imports System.Security.Cryptography.X509Certificates
Imports System.Security.Permissions
Imports System.Collections

Namespace SDKSample
	Friend Class PackageDigitalSignatureSample
		...
		Public Shared Sub Main()
			Dim msg As String
			msg = "Click OK to create '" & _packageFilename & "' " & vbLf & "with X.509 digitally signed parts."
		...
		Private Shared Sub CreatePackage(ByVal packageFilename As String)
			...
			Using package As Package = Package.Open(packageFilename, FileMode.Create)
				...
				package.CreateRelationship(packagePartDocument.Uri, TargetMode.Internal, _samplePackageRelationshipType)
			...
			End Using ' end:using (package) - Close and dispose 'package'
		...
		Private Shared Sub ExtractPackageParts(ByVal packageFilename As String)
			...
			Using package As Package = Package.Open(targetFilename, FileMode.Open, FileAccess.Read)
				' Validate the Package Signatures.
				' If all package signatures are valid, go ahead and unpack.
				If ValidateSignatures(package) Then
					Dim packagePartDocument As PackagePart = Nothing
					Dim packagePartResource As PackagePart = Nothing

					' Get the Package Relationships and look for
					' the Document part based on the RelationshipType.
					For Each relationship As PackageRelationship In package.GetRelationshipsByType(_samplePackageRelationshipType)
					...
					Next relationship ' end:if (ValidateSignature(package))
		...
		End Function ' end:ValidateSignatures()
		...
		End Sub ' end:SignAllParts()

	End Class ' end:class PackageDigitalSignatureSample
